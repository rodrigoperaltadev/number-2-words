...
function convert(number) {
  ...
  if (number < MILLION) {
    ...
    const thousandsText = thousands === 1 ? "one thousand" : `${convertBelowThousand(thousands)} thousand`;
    return rest2 === 0 ? thousandsText : `${thousandsText} ${convertBelowThousand(rest2)}`;
  }
  if (number < BILLION) {
...
function convert2(number) {
  ...
  if (number < MILLION) {
    const thousands = Math.floor(number / THOUSAND);
    const rest2 = number % THOUSAND;
    const thousandsText = thousands === 1 ? "mil" : `${convertBelowThousand2(thousands)} mil`;
    return rest2 === ZERO ? thousandsText : `${thousandsText} ${convertBelowThousand2(rest2)}`;
  }
  if (number < TRILLION) {
    const millions = Math.floor(number / MILLION);
    const rest2 = number % MILLION;
    const millionsText = millions === 1 ? "un mill\xF3n" : `${convert2(millions)} millones`;
  ...
  const billionsText = billions === 1 ? "mil millones" : `${convertBelowThousand2(billions)} mil millones`;
